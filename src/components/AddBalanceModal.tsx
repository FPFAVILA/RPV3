...
export const AddBalanceModal: React.FC<AddBalanceModalProps> = ({
  ...
  useEffect(() => {
    if (isOpen) {
      const initialAmount = suggestedAmount && suggestedAmount >= 9.80 ? suggestedAmount : 9.80;
            ...
            <div className="space-y-4">
              ...
              <div className="bg-accent/10 rounded-xl p-4 border border-accent/30">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { value: suggestedAmount && suggestedAmount >= 9.80 ? suggestedAmount.toFixed(2).replace('.', ',') : '9,80', games: suggestedAmount && suggestedAmount >= 9.80 ? 'Próximo jogo' : '2 jogos', popular: true },
                    { value: '19,60', games: '4 jogos', popular: false },
                      ...
                      {option.popular && (
                        <div className="absolute -top-1 left-1/2 transform -translate-x-1/2">
                          <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full">
                            {suggestedAmount && suggestedAmount >= 9.80 ? 'ESPECIAL' : 'POPULAR'}
                          </span>
                        </div>
                      )}
              ...
              <button
                onClick={generatePix}
disabled={!amount || parseFloat(amount.replace(',', '.')) < 9.80 || loading}
